...
flist = glob.glob("./sleepdata/*")
filehave = list(map(lambda x: x.split('/sd')[-1].split('.pkl')[0], flist))

datelist = list(set(datelist) - set(filehave))

for dd in datelist:
    requrl = 'https://api.fitbit.com/1.2/user/-/sleep/date/%s.json' %dd
    bresponse = requests.get(requrl, headers=headers, verify=False)
    if bresponse.ok:
        with open('./sleepdata/sd%s.pkl'%dd, 'wb') as f:
